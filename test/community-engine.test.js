...
import { validateProfile, sessionCandidates, sessionDocumentId, votingState, validateBallot, tallyBallots, resolvePublicPlayers, VOTING_WINDOW_MS, planEntryVoting } from '../community-engine.js';
...
test('equal points prioritize first-choice votes, then second-choice votes',()=>{
  const rank = selections => tallyBallots(session,selections.map((choices,index)=>({
    uid:`voter-${index}`,voterPlayerId:'non-participating-linked-player',
...
test('voting waits for four recorded candidates even for a linked non-participant',()=>{
  assert.equal(validateBallot({...valid,session:{...session,candidatePlayerIds:['b','c','d']}}),'sessionTooSmall');
});
