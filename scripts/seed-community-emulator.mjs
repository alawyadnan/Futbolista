...
for (const [role,email] of [['admin','admin@ftbll.live'],['linked','qa-linked@example.test'],['pending','qa-pending@example.test'],['rejected','qa-rejected@example.test']]) {
  ...
  const account=await response.json(); if(!response.ok)throw new Error(JSON.stringify(account));
  const verified=await fetch('http://127.0.0.1:9099/identitytoolkit.googleapis.com/v1/accounts:update?key=demo-key',{method:'POST',headers:{'content-type':'application/json',Authorization:'Bearer owner'},body:JSON.stringify({localId:account.localId,emailVerified:true})});
